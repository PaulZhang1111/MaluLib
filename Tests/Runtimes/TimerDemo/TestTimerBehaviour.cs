    ...
    public class TestTimerBehaviour : MonoBehaviour
    {
        ...
        public void StartTestTimer()
        {
            ...
            _testTimer = Timer.Register(
                ...
                {
                    UpdateText.text = string.Format("Update callback: {0:F2} seconds", secondsElapsed);
                },
        ...
        private void Update()
        {
            ...
            TimeElapsedText.text = string.Format("執行時間: {0:F2} seconds", _testTimer.GetTimeElapsed());
            TimeRemainingText.text = string.Format("剩餘時間: {0:F2} seconds", _testTimer.GetTimeRemaining());
            PercentageCompletedText.text = string.Format("執行時間比例: {0:F4}%",
                _testTimer.GetRatioComplete()*100);
            PercentageRemainingText.text = String.Format("剩餘時間比例: {0:F4}%",
                _testTimer.GetRatioRemaining()*100);
            NumberOfLoopsText.text = string.Format("# 循環次數: {0}", _numLoops);
            IsCancelledText.text = string.Format("是否取消: {0}", _testTimer.isCancelled);
            IsCompletedText.text = string.Format("是否完成: {0}", _testTimer.isCompleted);
            IsPausedText.text = String.Format("暫停: {0}", _testTimer.isPaused);
            IsDoneText.text = string.Format("完成: {0}", _testTimer.isDone);
